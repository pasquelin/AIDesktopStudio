 ...
 *
 * 🛑 It lives HERE and not beside `retarget.ts` because this is the only project of the repo whose
 * types carry the DOM, node AND the renderer alias at once: a renderer test may not touch
 * `node:fs` (invariant 1), and the node project has no `Worker` to compile `retarget.ts` against.
 */
import { readFileSync } from 'node:fs'
import { join } from 'node:path'
import { AnimationClip, AnimationMixer, Vector3, type Matrix4, type Object3D } from 'three'
...
async function loaded(file: string): Promise<Object3D> {
  ...
  // Copied into THIS realm's `ArrayBuffer`: handed node's, `GLTFLoader`'s `instanceof` reads false
  // under jsdom and it parses the binary as text, answering « Unsupported asset ».
...
function adapted(body: Object3D, file: Object3D): AnimationClip {
  ...
  // exactly as `retarget.worker.ts` has to spell it.
  ...
  const sampled = retargetClip(target, source, clipFromWire(first), options)
